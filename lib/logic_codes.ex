defmodule Logic do
  ...
  def logical_expression(expr) do
    ...
    new_expression =
      if length(special_patterns) != 0 do
      ...
      if String.contains?(exp, char_patterns) do
        new_expr =
          if char_patterns != [] do
            String.replace(
              exp,
              to_string(Enum.at(char_patterns, rem(i, length(char_patterns)))),
              to_string(v)
            )
          else
            exp
  ...
  def table(n, expr) do
    ...
    resultas =
      Enum.reduce(combinations, [], fn line, res ->
    ...
    case Agent.get(__MODULE__, &Map.get(&1, n)) do
      nil ->
        ...
        {first_half, second_half} =
          Enum.reduce(0..(length(previous_gray) - 1), {[], []}, fn index, {acc1, acc2} ->
